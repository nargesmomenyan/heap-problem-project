...
import assignTechnician from '~/components/technician/assign-technician.vue'
...
export default Vue.extend({
  ...
  async mounted() {
    if (this.currentUserInfo.RoleId == roleEnum.TechnicianSupervisor) {
      this.isTechnicianSupervisor = true
    }

    if (this.isTechnicianSupervisor) {
      this.actions.push({
        icon: 'check_circle_outline',
        title: this.$t('sample.assign-to-technician').toString(),
        cb: this.showDialogAssignToTechnician
      })
    } else {
      this.actions.push({
        icon: 'check_circle_outline',
        title: this.$t('sample.request-assign-to-other-technician').toString(),
        cb: this.showDialogRequestAssignToOtherTechnician
      })
    }
  ...
  methods: {
    row_clicked(item) {
      if (item.SampleStatus == sampleStatusEnum.Processing) {
        if (item.AssigneeId == this.currentUserInfo.UserId) {
          this.$router.push(
             `/technicians/receptions/${this.receptionId}/samples/${item.Id}?sampleType=${item.Type}`
          )
        } else {
    ...
    showDialogAssignToTechnician(item) {
      this.$dialog
        .show({
    ...
    async showDialogRequestAssignToOtherTechnician(item) {
